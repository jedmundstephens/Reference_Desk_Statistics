...
class PHPGraphLib {
	...
	var $multi_offset_three = 50; //percent of bar width
	var $multi_offset_four = 50; //percent of bar width
	var $multi_offset_five = 6; //percent of bar width
	var $multi_offset_six = 5; //percent of bar width
	...
	function setupData() {
		...
		if ($unit_width < 1) {	
			...
			$this->bar_width= 1.5 * $unit_width;
			$this->space_width = $unit_width*1.5;		
	...
	function generateBars() {
		...
		if ($this->bool_multi_offset) {
			...
			switch ($this->data_set_count) {
				case 2: $dataset_offset = $this->bar_width * ($this->multi_offset_two / 50); break;
				case 3: $dataset_offset = $this->bar_width * ($this->multi_offset_three / 50); break;
				case 4: $dataset_offset = $this->bar_width * ($this->multi_offset_four / 50); break;
				case 5: $dataset_offset = $this->bar_width * ($this->multi_offset_five / 50); break;
				case 6: $dataset_offset = $this->bar_width * ($this->multi_offset_six / 40); break;
				
			}
		}
		foreach ($this->data_array as $data_set_num => $data_set) {
			$lineX2 = NULL;
			//the next line was modified to move the first bar closer to the y axis.
			$xStart = $this->y_axis_x1 + ($this->space_width - 10);
	...
	function determineAxisMarkerScale($max, $min, $axis = 'y') {
		...
		if($this->data_interval!=0){
$result=$this->data_interval;
}
	...
	function addData($data, $data2 = '', $data3 = '', $data4 = '', $data5 = '', $data6 = '') {
		...
		if (is_array($data6)){ $this->data_array[]=$data6; }
		//assess data
...
$this->data_interval = $intervals;
	...
	function setBarColor($color, $color2 = '', $color3 = '', $color4 = '', $color5 = '', $color6 = '') {
...
if (!empty($color6)) {
			$this->setGenericColor($color6, '$this->multi_bar_colors[]', "Bar color 6 not specified properly.");
		}			
	}
	...
	function setLineColor($color, $color2 = '', $color3 = '', $color4 = '', $color5 = '', $color6 = '') {
		...
		if (!empty($color6)) {
			$this->setGenericColor($color6, '$this->line_color[]', "Line color 6 not specified properly.");	
		}
	}
	...
	function setLegendTitle($title, $title2 = '', $title3 = '', $title4 = '', $title5 = '', $title6 = '') {
		if (!empty($title)) { 		
			$len = strlen($title);
		...
		if (!empty($title6)) { 
			$len = strlen($title6);
			if ($len > $this->legend_max_chars){ 
				$title4 = substr($title6, 0, $this->legend_max_chars); 
				$this->legend_total_chars[] = $this->legend_max_chars;
			}
			else {$this->legend_total_chars[] = $len;}
			$this->legend_titles[] = $title6; 
		}
